...
class Servicios extends Conectar {
    ...
    public function insertServicio($name,$desc) {
        $this->col = implode(",",$this->column);

        // echo $this->col;
        // echo $this->val;
        $sql = "INSERT INTO {$this->table} ({$this->pkey},{$this->col}) VALUE (NULL,'$name','$desc')";
        // echo $sql;
        $this->db->query($sql);
    }

    public function updateServicio($value,$name,$desc)  {
        ...
        $this->values[] = $this->column[1] ."='". $desc ."'";
